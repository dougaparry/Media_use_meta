...
## Use correlations
```{r}
use_corr_data <- escalc(measure="ZCOR", 
                    ri=Correlation_r, 
                    ni=N, 
                    data=use_corr_data, 
...
```{r}
prob_use_corr_data <- escalc(measure="ZCOR", 
                    ri=Correlation_r, 
                    ni=N, 
                    data=prob_use_corr_data, 
                    slab=paste(Authors, Year, sep=", "))
```
...
Convert from z to r
```{r}
psych::fisherz2r(0.398) 
psych::fisherz2r(0.345) 
psych::fisherz2r(0.449) 
```
...
lower_conf <- (2*0.3319339)/(sqrt(1-0.3319339^2))

#upper confidence interval
upper_conf <- (2*0.4210767)/(sqrt(1-0.4210767^2))

#effect size
ES_r_to_d <- (2*0.3782364)/(sqrt(1-0.3782364^2)) 
...
rm(use_corr_data_no_outlier) #remove redundant dataframe
```
Convert from z to r
```{r}
psych::fisherz2r(0.393) 
psych::fisherz2r(0.344) 
psych::fisherz2r(0.443) 
```
...
rm(use_corr_data_Pub)
```

```{r}
psych::fisherz2r(0.411) 
psych::fisherz2r(0.351) 
psych::fisherz2r(0.471) 
```
...
rm(use_corr_data_UnPub)
```

```{r}
psych::fisherz2r(0.318) 
psych::fisherz2r(0.209) 
psych::fisherz2r(0.428) 
```
...
```{r}
psych::fisherz2r(-0.0832) 
psych::fisherz2r(-0.210) 
psych::fisherz2r(0.0432) 
```
...
rm(use_corr_data_non_digitised)
```

```{r}
psych::fisherz2r(0.405) 
psych::fisherz2r(0.354) 
psych::fisherz2r(0.455) 
```
...
segments(transf.rtoz(0.5), -5, transf.rtoz(0.5), 68, lty="dotted") #extra line at r=0.5
}
```
...
rm(use_corr_data_Phone)
```

```{r}
psych::fisherz2r(0.409) 
psych::fisherz2r(0.327) 
psych::fisherz2r(0.491) 
```
...
rm(use_corr_data_Social_Media)
```

```{r}
psych::fisherz2r(0.372) 
psych::fisherz2r(0.276) 
psych::fisherz2r(0.469) 
```
...
```{r}
psych::fisherz2r(0.395) 
psych::fisherz2r(0.336) 
psych::fisherz2r(0.454) 
```
...
rm(use_corr_data_Single)
```

```{r}
psych::fisherz2r(0.41) 
psych::fisherz2r(0.357) 
psych::fisherz2r(0.464) 
```
...
use_cor_scale_model
rm(use_corr_data_Scale)
```

```{r}
psych::fisherz2r(0.25) 
psych::fisherz2r(-0.0036) 
psych::fisherz2r(0.504) 
```
...
rm(use_corr_data_Duration)
```

```{r}
psych::fisherz2r(0.403) 
psych::fisherz2r(0.346) 
psych::fisherz2r(0.459) 
```
...
rm(use_corr_data_Volume)
```

```{r}
psych::fisherz2r(0.36) 
psych::fisherz2r(0.243) 
psych::fisherz2r(0.468) 
```
...
# Transform from log
```{r}
log_transform_mod <- predict(rq2_model, digits=7, transf = exp) 
log_transform_mod
```

# Use robu to calculate RVE
```{r}
res <- robu(formula = yi ~ 1, var.eff.size = vi, studynum = Study_ID, modelweights = "CORR", rho = 0.8, small = TRUE, data = acc_data)

res #note, these have not been transformed from log to exp
```
...
Because only sufficient data for a single moderator level, will not perform moderator analysis, but will perform a subgroup analysis
```{r}
...
# Perform Fisher's z-to-r for reporting
```{r}
h2_ztor_model <- predict(h2_model, digits=7, transf = transf.ztor) 
h2_ztor_model
```
...
```{r}
psych::fisherz2r(0.251)
psych::fisherz2r(0.199)
psych::fisherz2r(0.302)
```
...
#lower confidence interval
lower_conf <- (2*0.1964141)/(sqrt(1-0.1964141^2))

#upper confidence interval
upper_conf <- (2*0.2931418)/(sqrt(1-0.2931418^2))

#effect size
ES_r_to_d <- (2*0.2458584)/(sqrt(1-0.2458584^2)) 
...
rm(prob_use_corr_Pub)
```

```{r}
psych::fisherz2r(0.256) 
psych::fisherz2r(0.196) 
psych::fisherz2r(0.316) 
```

Note, for the below there is an insufficent number of unpublished studies to conduct the analysis with robu. Therefore, will use rma.uni.
```{r}
...
#moderator analysis for publication status
```{r}
prob_use_cor_pub_status_moderator_model <- robu(formula = yi ~ Source_Type, var.eff.size = vi, studynum = Study_ID, modelweights = "CORR", rho = 0.8, small = TRUE, data = prob_use_corr_data)
prob_use_cor_pub_status_moderator_model
```
